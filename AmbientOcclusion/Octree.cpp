	...
	else
	{
		...
		for(int i = 0; i < 8; ++i)
		{
			...
			for(size_t j = 0; j < triCount; ++j)
			{
				...
				if(MathHelper::IntersectTriangleAxisAlignedBox(v0, v1, v2, &subbox[i]))
				{
...
bool Octree::RayOctreeIntersect(OctreeNode* parent, FXMVECTOR rayPos, FXMVECTOR rayDir)
{
	...
	if( !parent->IsLeaf )
	{
		for(int i = 0; i < 8; ++i)
		{
			...
			float t;
			if( MathHelper::IntersectRayAxisAlignedBox(rayPos, rayDir, &parent->Children[i]->Bounds, &t) )
			{
	...
	else
	{
		...
		for(size_t i = 0; i < triCount; ++i)
		{
			...
			float t;
			if( MathHelper::IntersectRayTriangle(rayPos, rayDir, v0, v1, v2, &t) )
				return true;
		}
