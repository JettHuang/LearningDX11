...
class TreeBillboardApp : public FD3DApp
{
public:
...
TreeBillboardApp::TreeBillboardApp(HINSTANCE hInstance)
	: FD3DApp(hInstance), mLandVB(0), mLandIB(0), mWavesVB(0), mWavesIB(0), mBoxVB(0), mBoxIB(0), mTreeSpritesVB(0),
...
bool TreeBillboardApp::Init()
{
	...
	HR(CreateDDSTextureFromFile(md3dDevice,
		L"Textures/grass.dds", NULL, &mGrassMapSRV, 0, NULL));

	HR(CreateDDSTextureFromFile(md3dDevice,
		L"Textures/water2.dds", NULL, &mWavesMapSRV, 0, NULL));

	HR(CreateDDSTextureFromFile(md3dDevice,
		L"Textures/WireFence.dds", NULL, &mBoxMapSRV, 0, NULL));

	std::vector<std::wstring> treeFilenames;
	treeFilenames.push_back(L"Textures/tree1.dds"); // Must be the same texture format
...
void TreeBillboardApp::DrawTreeSprites(CXMMATRIX viewProj)
{
	...
	ID3DX11EffectTechnique* treeTech = NULL;
	switch (mRenderOptions)
	{
	case RenderOptions::Lighting:
